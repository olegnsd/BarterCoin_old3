<div class="pageTitle"><h2>Активация карты</h2></div>
...
<script type="text/javascript">
	jQuery(function($)
	{
		...
		$('[name=phone]').inputmasks(
		{
			...
			match: /[0-9]/,
			replace: '#',
			list: $.masksSort($.masksLoad('/js/json/phone-codes.json'), ['#'], /[0-9]|#/, 'mask'),
			listKey: 'mask',
			onMaskChange: function(maskObj, determined)
			{
				var hint = 'Номер телефона';
				if (determined) hint = (maskObj.desc_ru && maskObj.desc_ru != '') ? maskObj.name_ru + ' (' + maskObj.desc_ru + ')' : maskObj.name_ru;
				$('.form-control-label').html(hint);
			}
		});
	});
</script>
...
.form-control-label {position: absolute; left: 8px; top: -7px; line-height: 14px; margin: 0px; padding: 0px 5px; font-size: 12px; font-weight: normal;
	color: #888888; background: #ffffff;
}
</style>
<form method="POST" action="<?=$baseHref;?>do/" target="_blank">
	...
	<div class="col-md-5 col-xs-12">
		<div class="card from">
			<div class="row">
				...
				<div class="col-xs-offset-1 col-xs-10 form-inline">
					<input type="num" name="frommonth" class="form-control" style="width:90px;display: inline;" placeholder="Месяц" required=""> / <input type="num" name="fromyear" class="form-control" style="width:90px;display: inline;" placeholder="Год" required="">
					<div style="float:right;">СVC: <input type="num" name="fromcvc" class="form-control" style="width:90px;display: inline;" placeholder="cvc" required=""></div>
				</div>
	...
	<div class="col-md-5 col-xs-12 text-center">
		...
		<div class="relative">
			<input type="text" name="phone" class="form-control cardnum" required>
			<label class="form-control-label"></label>
		</div>
	</div>
	<div class="clearfix"></div>
	<div style="text-align:center;padding-top:20px;">
		<button type="submit" id="submit123" class="btn btn-success btn-lg">Активировать карту <i class="fa fa-chevron-right"></i></button>
	</div>
	<br>
	<br>
</form>
